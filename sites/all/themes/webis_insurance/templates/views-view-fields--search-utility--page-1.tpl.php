...
<?php
  ...
  $node = node_load($nid);
	...
	$next_coverage_link = "";
	$next_coverage = site_helper_next_converage_by_node($node, $amount);
	if(!empty($next_coverage)){
		//Building next converage query.
		$next_coverage_query = "";
		$current_page = "";
		foreach($_GET as $q => $q_value)
		{
			if($q == "q"){
				$current_page = $q_value;
				continue;
			}

			if($q == "amount"){
				$q_value = $next_coverage;
			}
			$next_coverage_query .= $q."=".$q_value."&";
		}
		$next_coverage_link = '<span class="next_coverage_price_search">'.l( t("Compare with $@d", array("@d"=>$next_coverage)), $current_page, array(
				"query" => $next_coverage_query,
				"attributes" => array(
					"target" => "_blank",
					"title" => t("Compare with next level converage."),
				),
			)
		).'</span>';		
	}
  ...
  $age = site_helper_get_which_age_by_node($node, $birthday, $start);
	...
	/*print site_helper_build_price_search($node, $amount, $age, $days);
	return;
*/
	...
	?>
		...
		?>
			<td rowspan="<?php print $rowspan;?>" class="insurance_company">
				<div align="center" >
					<div class="product_image"><?php print $fields['field_image_fid']->content;?></div>
					<div class="product_title"><?php print $fields["title"]->content;?></div><br>
					<?php 
